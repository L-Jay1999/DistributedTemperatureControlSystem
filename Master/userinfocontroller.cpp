...
std::tuple<bool,QString> UserInfoController::AddUser(const QString& RoomID,const QString& UserID)
{
    /*if(dba.isConnected() == true)//判断是否连接至数据库
    {
        if(dba.hasUser(RoomID,UserID) == false)//判断是否已存在该用户
        {
            ...
            if(res == true)
            {
                return (std::tuple<bool,Qstring>)(true,"添加成功");
            }else
            {
                return (std::tuple<bool,Qstring>)(false,"添加失败：数据库插入失败");
            }
        }else
        {
            return (std::tuple<bool,Qstring>)(false,"添加失败：已存在的用户");
        }
    }else
    {
        return (std::tuple<bool,Qstring>)(false,"添加失败：数据库无连接");
    }*/
}
std::tuple<bool,QString> UserInfoController::DeleteUser(const QString& RoomID,const QString& UserID)
{
    /*if(dba.isConnected() == true)//判断是否连接至数据库
    {
        if(dba.hasUser(RoomID,UserID) == true)//判断是否已存在该用户
        {
            ...
            if(res == true)
            {
                return (std::tuple<bool,Qstring>)(true,"删除成功");
            }else
            {
                return (std::tuple<bool,Qstring>)(false,"删除失败：数据库删除失败");
            }
        }else
        {
            return (std::tuple<bool,Qstring>)(false,"删除失败：不存在该用户");
        }
    }else
    {
        return (std::tuple<bool,Qstring>)(false,"删除失败：数据库无连接");
    }*/
}
...
QString UserInfoController::GetUser()
{
    ...
    if(UserInfo.first == true)
    {
        ...
        {
            result = result + std::get<0>(*it) + "\t" + std::get<1>(*it) + "\t" /
                    + QString::number(std::get<2>(*it)) + "\t" + QString::number(std::get<3>(*it)) + "\n";
        }
        return result;
    }else
    {
        return (QString)"用户信息获取失败";
    }*/
}
